...
class ReportsListActivity : AppCompatActivity() {
    ...
    private fun setupRecyclerView() {
        adapter = ReportsAdapter(emptyList(), emptyList()) { selectedReport ->
                ...
                when (resource) {
                    ...
                    is Resource.Success -> {
                        ...
                        if (reports.isEmpty()) {
                            ...
                            adapter.updateData(reports, patientNamesList)
                        }
                    }
                    is Resource.Error -> {
                        progressBar.visibility = View.GONE
                        tvEmptyState.visibility = View.VISIBLE
                        tvEmptyState.text = resource.message ?: "Failed to load reports."
                    }
                }
            }
        }
